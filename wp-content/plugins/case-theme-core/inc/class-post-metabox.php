...
class CT_Post_Metabox
{
    ...
    function __construct()
    {
        ...
        do_action('ct_post_metabox_register', $this);
        ...
        if (!empty($this->panels)) {
            ...
            add_action('hidden_meta_boxes', array($this, 'ct_set_user_metaboxes'));
        }
    }
    ...
    function admin_init()
    {
        ...
        if ('post.php' == $pagenow) {
            $this->notices = get_transient('ct-post-metabox-transients');

            // If the transient exists then we override it with an empty value
            if (false !== $this->notices) {
                set_transient('ct-post-metabox-transients', '');
            } else {
    ...
    {
        $this->default_args['opt_name'] = isset($this->default_args['opt_name']) ? $this->default_args['opt_name'] . '_post_metabox' : 'ct_post_metabox';
        $this->default_args['display_name'] = esc_html__('Settings', CT_TEXT_DOMAIN);
    ...
    function add_section($post_type, $section = array())
    {
        ...
        foreach ($section['fields'] as $fkey => $field) {
            ...
            if (in_array($field['id'], $this->field_ids[$post_type])) {
                trigger_error(esc_html__('The field with id', CT_TEXT_DOMAIN) . ' ' . esc_html($field['id']) . ' ' . esc_html__('for post type', CT_TEXT_DOMAIN) . ' ' . esc_html($post_type) . ' ' . esc_html__('is already registered.', CT_TEXT_DOMAIN));
        ...
        if (!empty($section['id'])) {
            if ($this->section_exist($section['id'], $post_type)) {
                trigger_error(esc_html__('Section', CT_TEXT_DOMAIN) . ' ' . esc_html($field['id']) . ' ' . esc_html__('for post type', CT_TEXT_DOMAIN) . ' ' . esc_html($post_type) . ' ' . esc_html__('is already registered.', CT_TEXT_DOMAIN));
    ...
    function add_meta_boxes()
    {
        ...
        foreach ($this->panels as $post_type => $panel) {
            if ($post_type !== 'post') {
                ...
                if (in_array(str_replace('ct_pf_', '', $new_post_type), $post_formats) !== false) {
                    $new_post_type = 'post';
                    $priority = 'core';
                }
                add_meta_box(
    ...
    function ct_set_user_metaboxes($hidden)
    {
...
//        die();
        ...
        wp_enqueue_style('ct-metabox', CT_URL . '/assets/css/metabox' . Redux_Functions::isMin() . '.css', array(), self::$version, 'all');
        wp_enqueue_script('ct-metabox', CT_URL . '/assets/js/metabox' . Redux_Functions::isMin() . '.js', array(
            'jquery',
            'redux-js'
        ), self::$version, 'all');
    ...
    {
        return CT_PATH . '/templates/panel-post-meta';
    }
    ...
    function save_meta_boxes($post_id, $post)
    {
        ...
        if (!empty($post_format)) {
            $ct_pf_panel = 'ct_pf_' . $post_format;
            $post_format_type = !empty($_POST['post_format_' . $post_format]) ? $_POST['post_format_' . $post_format] : '';
            if (in_array($ct_pf_panel, $this->post_types) && !empty($_POST[$this->panels[$ct_pf_panel]['args']['opt_name']]) && !empty($post_format_type)) {
                $opt_name = $this->panels[$ct_pf_panel]['args']['opt_name'];
                $sections_post_format = $this->get_opt_sections($ct_pf_panel);
                $args_post_format = $this->get_opt_args($ct_pf_panel);
                $data_to_save_pfm = array();
                $data_to_compare_pfm = $this->get_opt_defaults($ct_pf_panel);
        ...
        if (!empty($redux->errors) || !empty($redux->warnings)) {
            ...
            set_transient('ct-post-metabox-transients', $notices);
        }
    ...
    function admin_notices()
    {
        ...
        if (!empty($this->notices['errors'])) {
            ...
            printf(
                ...
                esc_html__('error(s)', CT_TEXT_DOMAIN),
                esc_html__('were found! Some data might not be saved.', CT_TEXT_DOMAIN)
            );
            echo '</p></div>';
        }
        ...
        if (!empty($this->notices['warnings'])) {
            ...
            printf(
                ...
                esc_html__('warning(s)', CT_TEXT_DOMAIN),
                esc_html__('were found! Some data might not be saved.', CT_TEXT_DOMAIN)
            );
    ...
    protected function generate_output_css( $post, $options ){
        ...
        printf(
            ...
            esc_attr('ct-' . $post->post_type . '-dynamic-css'),
            $redux->outputCSS
        );
    }
}