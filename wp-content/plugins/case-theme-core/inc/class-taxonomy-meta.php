...
class CT_Taxonomy_Meta
{
    ...
    function __construct()
    {
        ...
        do_action('ct_taxonomy_meta_register', $this);
    ...
    function admin_init()
    {
        ...
        if ('term.php' == $pagenow) {
            $this->notices = get_transient('ct-taxonomy-meta-transients');

            // If the transient exists then we override it with an empty value
            if (false !== $this->notices) {
                set_transient('ct-taxonomy-meta-transients', '');
            } else {
    ...
    {
        $this->default_args['opt_name'] = isset($this->default_args['opt_name']) ? $this->default_args['opt_name'] . '_taxonomy_metabox' : 'ct_taxonomy_metabox';
    ...
    function add_section($taxonomy, $section = array())
    {
        ...
        foreach ($section['fields'] as $fkey => $field) {
            ...
            if (in_array($field['id'], $this->field_ids[$taxonomy])) {
                trigger_error(sprintf(esc_html__('The field with id %1$s for taxonomy %2$s is already registered.', CT_TEXT_DOMAIN), esc_html($field['id'], ETCTEXT_DOMAIN), esc_html($taxonomy, ETCTEXT_DOMAIN)));
        ...
        if (!empty($section['id'])) {
            if ($this->section_exist($section['id'], $taxonomy)) {
                trigger_error(sprintf(esc_html__('Section %1$s for taxonomy %2$s is already exist.', CT_TEXT_DOMAIN), esc_html($field['id']), esc_html($taxonomy)));
                return;
            } else {
    ...
    {
        wp_nonce_field('ct_taxonomy_metabox_nonce_action', 'ct_taxonomy_metabox_nonce');
    ...
    function panel_scripts()
    {
        ...
        wp_enqueue_script('ct-metabox', CT_URL . '/assets/js/metabox' . Redux_Functions::isMin() . '.js', array('jquery', 'redux-js'), self::$version, 'all');
    ...
    protected function save_term_meta($term_id, $tt_id, $taxonomy, $transient = false)
    {
        ...
        if (empty($_POST[$opt_name]) || empty($_POST['ct_taxonomy_metabox_nonce']) || !wp_verify_nonce($_POST['ct_taxonomy_metabox_nonce'], 'ct_taxonomy_metabox_nonce_action')) {
            return;
        }
        ...
        if ($transient) {
            ...
            if (!empty($redux->errors) || !empty($redux->warnings)) {
                ...
                set_transient('ct-taxonomy-metabox-transients', $notices);
            }
        }
    }
...
//        }
    ...
    protected function generate_output_css($taxonomy, $options)
    {
        ...
        echo '<style id="ct-' . $taxonomy . '-dynamic-css" data-type="redux-output-css">' . $redux->outputCSS . '</style>';
    }
}
