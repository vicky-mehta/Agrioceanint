...
<div id="<?php echo esc_attr($html_id) ?>" class="ct-service-carousel13 dot-style-u8 ct-slick-slider">
                    ...
                    <div class="grid-item-inner <?php echo esc_attr($ct_animate); ?> <?php if (has_post_thumbnail($post->ID) && wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), false)) { echo 'active-featured'; } ?>">
                        <div class="item--holder">
                            <?php if(!empty($service_icon)) : ?>
                                <div class="item--icon icon-abs"><i class="<?php echo esc_attr($service_icon); ?>"></i></div>
                            <?php endif; ?>
                        ...
                        <div class="item--holder-hover">
                            ...
                            <?php if($show_button == 'true') : ?>
                                <div class="item-readmore">
                                    <a href="<?php if(!empty($service_custom_link)) { echo esc_url($service_custom_link); } else { echo esc_url(get_permalink( $post->ID )); } ?>"><i class="flaticonv7-right-1"></i></a>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
